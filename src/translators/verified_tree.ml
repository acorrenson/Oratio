...
type thm =
  | Leaf of Rules.thm
  | Node of string * thm list * Rules.thm
...
let intro_bot t1 t2 =
  Node ("intro_bot", [t1; t2], Rules.intro_bot (get_rthm t1) (get_rthm t2))

let intro_impl t p =
  Node ("intro_impl", [t], Rules.intro_impl (get_rthm t) p)

let intro_and t1 t2 =
  Node ("intro_and", [t1; t2], Rules.intro_and (get_rthm t1) (get_rthm t2))

let intro_or_l t p =
  Node ("intro_or_l", [t], Rules.intro_or_l (get_rthm t) p)

let intro_or_r t p =
  Node ("intro_or_r", [t], Rules.intro_or_r (get_rthm t) p)

let elim_bot t p =
  Node ("elim_bot", [t], Rules.elim_bot (get_rthm t) p)

let elim_impl t1 t2 =
  Node ("elim_impl", [t1; t2], Rules.elim_impl (get_rthm t1) (get_rthm t2))

let elim_and_l t =
  Node ("elim_and_l", [t], Rules.elim_and_l (get_rthm t))

let elim_and_r t =
  Node ("elim_and_r", [t], Rules.elim_and_r (get_rthm t))

let elim_or t1 t2 t3 =
  Node ("elim_or", [t1; t2; t3], Rules.elim_or (get_rthm t1) (get_rthm t2) (get_rthm t3))
